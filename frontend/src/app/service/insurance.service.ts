...
export class InsuranceService {
  ...
  getInsurances(): Observable<Insurance[]> {
    ...
    return this.http.get<Insurance[]>(this.baseUrl, { headers })
      .pipe(
        catchError(error => {
  ...
  getInsuranceById(id: number): Observable<Insurance> {
    const headers = this.getAuthHeaders();
    return this.http.get<Insurance>(`${this.baseUrl}/${id}`, { headers })
      .pipe(
        catchError(error => {
  ...
  updateInsurance(id: number, updatedInsurance: Insurance): Observable<Insurance> {
    const headers = this.getAuthHeaders();
    return this.http.put<Insurance>(`${this.baseUrl}/${id}`, updatedInsurance, { headers })
      .pipe(
        catchError(error => {
